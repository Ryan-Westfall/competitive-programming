# 4046. Minimum Cost Path With At Most K Turns (Hard)

**Slug:** `minimum-cost-path-with-at-most-k-turns`
**ID:** 4046
**Difficulty:** Hard
**Tags:** N/A
**Language:** Python3
**Runtime:** 5374 ms (10.4%)
**Memory:** 71.8 MB (31.1%)
...
## Hints
1. <strong>1a (Dynamic Programming).</strong> Track the minimum cost for each cell, number of turns used, and direction of the last move. Initialize paths with no turns by moving straight from the starting cell.
2. <strong>1b (Dynamic Programming).</strong> To arrive at a cell in direction <code>d</code>, either continue in direction <code>d</code> without adding a turn, or change from another direction and add one turn.
3. <strong>1c (Dynamic Programming).</strong> Process turn counts in increasing order. For each direction, sweep the grid in that direction so that straight-move transitions use states already computed in the current layer. This gives <code>O(m * n * (k + 1))</code> time.
4. <strong>2a (Dijkstra).</strong> Treat each combination of cell, last move direction, and number of turns used as a shortest-path state. A move costs the value of the destination cell.
5. <strong>2b (Dijkstra).</strong> Changing direction adds one turn, while the first move adds none. Start with cost <code>grid[0][0]</code>, discard states using more than <code>k</code> turns, and run Dijkstra's algorithm.
## Solution
