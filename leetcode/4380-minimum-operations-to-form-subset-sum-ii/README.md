...
**Slug:** `minimum-operations-to-form-subset-sum-ii`
**ID:** 4041
**Difficulty:** Hard
**Tags:** N/A
**Language:** Python3
**Runtime:** 14629 ms (32.0%)
**Memory:** 19.6 MB (40.5%)
...
## Hints
1. In a shortest sequence of operations on one element, a multiplication followed later by a division can be canceled when they become adjacent. This lets you characterize an optimal sequence as some number of divisions followed by some number of multiplications.
2. Enumerate the values of the form obtained by repeatedly dividing <code>x</code>, then repeatedly doubling the result, keeping only values at most <code>sum</code> and the minimum cost for each. Combine these per-element choices with knapsack DP.
## Solution
