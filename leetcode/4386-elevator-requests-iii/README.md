...
**ID:** 4027
**Difficulty:** Hard
**Tags:** Array, Dynamic Programming, Bit Manipulation, Sorting, Bitmask
**Language:** Python3
**Runtime:** 6583 ms (31.8%)
**Memory:** 452.1 MB (7.9%)
...
## Hints
1. <p>Since there are at most 16 requests, use bitmask dynamic programming over the set of requests already fulfilled.</p>
2. <p>Let <code>dp[mask][i]</code> be the minimum time at which all requests in <code>mask</code> can be fulfilled with request <code>i</code> being the last one visited.</p>
3. <p>If the elevator is at <code>floor[i]</code> at time <code>t</code> and next serves request <code>j</code>, the earliest time it can fulfill <code>j</code> is <code>max(t + abs(floor[i] - floor[j]), arrival[j])</code>.</p>
4. <p>Initialize each single-request state similarly using the distance from <code>start</code>, then take the minimum over all states containing every request.</p>
## Solution
