# 4044. Count Good Cyclic Rotations (Medium)
...
## Hints
1. Let <code>total</code> be the sum of all elements and <code>first</code> be the sum of the first half of a rotation. The rotation is good exactly when <code>first &gt; total - first</code>.
2. Maintain <code>first</code> using a circular sliding window of length <code>n / 2</code>. Moving to the next rotation removes one element from the window and adds one element.
## Solution
